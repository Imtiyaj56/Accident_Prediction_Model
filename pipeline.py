...
#prediction function_______________________

def predict_injury(Day_of_week, Age_band_of_driver, Sex_of_driver, Educational_level, Vehicle_driver_relation,
    ...
    # Your prediction code here, convert in 2d array using numpy
    ...
    #assuming pipeline is your training model pipeline
    #predict result based on user input, and it also do every preprocessing needed to data through pipeline
    results = pipe.predict(features)  
    return results



#create streamlit app___________________________________
#=====>


#load the pipe and image
pipe = pickle.load(open('my pipe.pkl', 'rb'))
img = Image.open('accident_img.png')


#create app
st.title('Accident Prediction Model')
st.subheader("DRIVE SAFELY, REACH SAFELY")
st.image(img, use_container_width=True)


#sidebar for user inputs
with st.sidebar:
    st.write("INPUT THE DATA HERE !!")
...
if st.sidebar.button('PREDICT'):  #called pred fun, and save all input and predict the result using pipeline
    predicted_result = predict_injury(
